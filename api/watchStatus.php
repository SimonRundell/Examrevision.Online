...
$query = "SELECT * FROM tblstatus WHERE quizCode = ? AND lastUpdate = (
    SELECT MAX(lastUpdate) FROM tblstatus WHERE quizCode = ? AND studentID = tblstatus.studentID
)";
...
$stmt->bind_param("ss", $receivedData['quizCode'], $receivedData['quizCode']);
